...
def convert_to_hex_8_bytes(arg):
    ...
    if (int_rs <= 0xf): # one nibble
        fstring = '0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x0' + rs[-1:]
    elif (int_rs <= 0xff): # two nibbles
        fstring = '0x00 0x00 0x00 0x00 0x00 0x00 0x00 ' + rs
    elif (int_rs <= 0xfff): # three nibbles
        fstring = '0x00 0x00 0x00 0x00 0x00 0x00 0x0' + rs[-3:-2] + ' 0x' + rs[-2:]
    elif (int_rs <= 0xffff): # four nibbles
        fstring = '0x00 0x00 0x00 0x00 0x00 0x00 0x'+ rs[-4:-2] + ' 0x' + rs[-2:]
    elif (int_rs <= 0xfffff): # five nibbles
        fstring = '0x00 0x00 0x00 0x00 0x00 0x0'+ rs[-5:-4] + ' 0x' + rs[-4:-2] + ' 0x' + rs[-2:]
    elif (int_rs <= 0xffffff): # six nibbles
        fstring = '0x00 0x00 0x00 0x00 0x00 0x'+ rs[-6:-4] + ' 0x' + rs[-4:-2] + ' 0x' + rs[-2:]
    elif (int_rs <= 0xfffffff): # seven nibbles
        fstring = '0x00 0x00 0x00 0x00 0x0' + rs[-7:-6] + ' 0x'+ rs[-6:-4] + ' 0x' + rs[-4:-2] + ' 0x' + rs[-2:]
    elif (int_rs <= 0xffffffff): # eight nibbles
        fstring = '0x00 0x00 0x00 0x00 0x' + rs[-8:-6] + ' 0x'+ rs[-6:-4] + ' 0x' + rs[-4:-2] + ' 0x' + rs[-2:]
    elif (int_rs <= 0xfffffffff): # nine nibbles
        fstring = '0x00 0x00 0x00 0x0' + rs[-9:-8] + ' 0x' + rs[-8:-6] + ' 0x'+ rs[-6:-4] + ' 0x' + rs[-4:-2] + ' 0x' + rs[-2:]
    elif (int_rs <= 0xffffffffff): # ten nibbles
        fstring = '0x00 0x00 0x00 0x' + rs[-10:-8] + ' 0x' + rs[-8:-6] + ' 0x'+ rs[-6:-4] + ' 0x' + rs[-4:-2] + ' 0x' + rs[-2:]
    elif (int_rs <= 0xfffffffffff): # eleven nibbles
        fstring = '0x00 0x00 0x0' + rs[-11:-10] + ' 0x' + rs[-10:-8] + ' 0x' + rs[-8:-6] + ' 0x'+ rs[-6:-4] + ' 0x' + rs[-4:-2] + ' 0x' + rs[-2:]
    elif (int_rs <= 0xffffffffffff): # twelve nibbles
        fstring = '0x00 0x00 0x' + rs[-12:-10] + ' 0x' + rs[-10:-8] + ' 0x' + rs[-8:-6] + ' 0x'+ rs[-6:-4] + ' 0x' + rs[-4:-2] + ' 0x' + rs[-2:]
    elif (int_rs <= 0xfffffffffffff): # thirteen nibbles
        fstring = '0x00 0x0' + rs[-13:-12] + ' 0x' + rs[-12:-10] + ' 0x' + rs[-10:-8] + ' 0x' + rs[-8:-6] + ' 0x'+ rs[-6:-4] + ' 0x' + rs[-4:-2] + ' 0x' + rs[-2:]
    elif (int_rs <= 0xffffffffffffff): # fourteen nibbles
        fstring = '0x00 0x' + rs[-14:-12] + ' 0x' + rs[-12:-10] + ' 0x' + rs[-10:-8] + ' 0x' + rs[-8:-6] + ' 0x'+ rs[-6:-4] + ' 0x' + rs[-4:-2] + ' 0x' + rs[-2:]
    elif (int_rs <= 0xfffffffffffffff): # fifteen nibbles
        fstring = '0x0' + rs[-15:-14] + ' 0x' + rs[-14:-12] + ' 0x' + rs[-12:-10] + ' 0x' + rs[-10:-8] + ' 0x' + rs[-8:-6] + ' 0x'+ rs[-6:-4] + ' 0x' + rs[-4:-2] + ' 0x' + rs[-2:]
    else: # sixteen nibbles
        fstring = '0x' + rs[-16:-14] + ' 0x' + rs[-14:-12] + ' 0x' + rs[-12:-10] + ' 0x' + rs[-10:-8] + ' 0x' + rs[-8:-6] + ' 0x'+ rs[-6:-4] + ' 0x' + rs[-4:-2] + ' 0x' + rs[-2:]
    return fstring
...
parser.add_argument("-c", "--camera", nargs='+',
                    help="select camera: from a1->a5, b1->b5 \
                                            or c1->c6",
                    action="store")
...
parser.add_argument("-u", "--ucid", help="ucid for setting", 
                    action="store", choices=['preview', 'hires'])
parser.add_argument("-f", "--focus", help="select focus distance for 35mm \
                                           cameras",
                    action="store")
...
parser.add_argument("-mul", "--multiplier", help="Multiplier for the fine nudge command",
                    action="store")
parser.add_argument("-csi", "--csi_interface", help="CSI interface",
                    action="store", nargs='+', choices=['0', '1'])
...
if args.camera:
    ...
    camera_string = ""
    if args.stream:
        camera_string = "adb shell \"echo " + str(byte_count) + " 0x0002 " + m_bitmask \
    ...
    if not args.pwm_test_cpld_mode_switch and \
       ...
       not args.cpld_stm_config:
        parser.print_help()

if args.ucid:
    if not args.camera:
        # Light active ucid
        str = "adb shell 'echo 2 0x1000 " + ucid_to_hex(args.ucid) + " > /sys/class/light_ccb/i2c_interface/i2c_w'"
...
if args.focus:
    # the focus argument is used to control the VCMs on the 35mm cameras. the
    # piezos on the 70mm and 150mm cameras are controlled separately. 

    fstring = ""
    byte_count = 3
    # check if we have non-35mm camera 
    for camera in args.camera:
        if camera not in atypecam:
            print "camera[s] are: " + str(args.camera)
            print "Use the lens argument for 70mm or 150mm cameras."
            exit()
        else:
    ...
    if args.ucid:
        ...
        focus_string = "adb shell \"echo " + str(byte_count + 2) + " 0x0048 " + m_bitmask + " " + ucid + " " + fstring + \
        " > /sys/class/light_ccb/i2c_interface/i2c_w\""
    else:    
        focus_string = "adb shell \"echo " + str(byte_count) + " 0x0048 " + m_bitmask + " " + fstring + \
        " > /sys/class/light_ccb/i2c_interface/i2c_w\""
...
if args.lens_manual:
    if (args.camera in atypecam):
        ...
        if args.direction and args.dutycycle and args.duration:
            ...
            lens_string = "adb shell \"echo 7 0x0043 " + m_bitmask + " "\
...
if args.mirror_manual:
    if (args.camera in atypecam):
        ...
        if args.direction and args.dutycycle and args.duration:
            ...
            dutycycle_string = convert_dutycycle(args.dutycycle)
            duration_string = one_byte_hex(args.duration)
            mirror_string = "adb shell \"echo 7 0x0047 " + m_bitmask + " "\
                           + direction_string + " " + dutycycle_string + \
                           " " + duration_string + \
                           " > /sys/class/light_ccb/i2c_interface/i2c_w\""
    print mirror_string
...
if args.exposure:
    if args.camera:
        ...
        if args.ucid:
            ucid = ucid_to_hex(args.ucid)
            exposure_string = "adb shell \"echo " + str(byte_count + 2) + " 0x0032 " + m_bitmask + " " + \
            ucid + " " + fstring + " > /sys/class/light_ccb/i2c_interface/i2c_w\""
        else:
            exposure_string = "adb shell \"echo " + str(byte_count) + " 0x0032 " + m_bitmask + " " + \
...
if args.gain:
    if args.camera:
        ...
        if args.ucid:
            ucid = ucid_to_hex(args.ucid)
            gain_string = "adb shell \"echo " + str(byte_count + 2) + " 0x0030 " + m_bitmask + " " + ucid + " "\
            + fstring + \
            " > /sys/class/light_ccb/i2c_interface/i2c_w\""
        else:                    
            gain_string = "adb shell \"echo " + str(byte_count) + " 0x0030 " + m_bitmask + " "\
            + fstring + \
            " > /sys/class/light_ccb/i2c_interface/i2c_w\""
        print gain_string
        execute(gain_string)
...
if args.calibrate:
    if (args.camera in atypecam):
        ...
        if args.camera:
            ...
            m_bitmask = handle_endianness(m_bitmask_str)
            print "m_bitmask = " + m_bitmask
            camera_string = "adb shell \"echo 4 0x0060 " + m_bitmask \
...
if args.pwm_test_cpld_mode_switch:
    ...
    debug_test_string = "adb shell \"echo 3 0x0090 0x02 0x06 " + cpld_mode_string + " > /sys/class/light_ccb/i2c_interface/i2c_w\"" 
    #b_module_string = "adb shell \"echo 3 0x0090 0x02 0x02 0xFF > /sys/class/light_ccb/i2c_interface/i2c_w\""
    #c_module_string = "adb shell \"echo 3 0x0090 0x02 0x03 0xFF > /sys/class/light_ccb/i2c_interface/i2c_w\""
...
#    if module[0] == 'b':
#        b_module_string = "adb shell \"echo 3 0x0090 0x02 0x02 " + one_byte_hex(module_int) + " > /sys/class/light_ccb/i2c_interface/i2c_w\""
#        c_module_string = "adb shell \"echo 3 0x0090 0x02 0x03 0x00 > /sys/class/light_ccb/i2c_interface/i2c_w\""
#    elif module[0] == 'c':
#        b_module_string = "adb shell \"echo 3 0x0090 0x02 0x02 0x00 > /sys/class/light_ccb/i2c_interface/i2c_w\""
#        c_module_string = "adb shell \"echo 3 0x0090 0x02 0x03 " + one_byte_hex(module_int) + " > /sys/class/light_ccb/i2c_interface/i2c_w\""
#    else:
#        print "Invalid input for module. Input B1-B5 or C1-C6. "
#        exit()

    # turn on ALL modules for now
    b_module_string = "adb shell \"echo 3 0x0090 0x02 0x02 0x1F > /sys/class/light_ccb/i2c_interface/i2c_w\""
    c_module_string = "adb shell \"echo 3 0x0090 0x02 0x03 0x3F > /sys/class/light_ccb/i2c_interface/i2c_w\""
...
#    output_string = "adb shell \"echo 6 0x0091 0x05 0x02 " + offset + " " + freq_str + " > /sys/class/light_ccb/i2c_interface/i2c_w\""
#    print output_string
#    execute(output_string)
        
    # adb shell "echo 3 0x0090 0x02 0x0a 0x00"
    ...
    for i in xrange(0, len(dutycycle_lists)):
        ...
        for j in xrange(0, 2): # two byte duty cycles
            ...
            output_string = "adb shell \"echo 3 0x0090 0x02 " + offset + " " + one_byte_hex(str(dc_list[j])) + " > /sys/class/light_ccb/i2c_interface/i2c_w\""
            print output_string
            execute(output_string)
...
#    #print collection
#
#    output_string = "adb shell \"echo 27 0x0091 0x18 0x02 " + offset + " " + collection + " > /sys/class/light_ccb/i2c_interface/i2c_w\""
#    print output_string
#    execute(output_string)
...
#
#    output_string = "adb shell \"echo 27 0x0091 0x18 0x02 " + "0x20" + " " + single_pair_string + " > /sys/class/light_ccb/i2c_interface/i2c_w\""
#    print output_string
#    execute(output_string)
#    output_string = "adb shell \"echo 11 0x0091 0x08 0x02 " + "0x50" + " " + double_pair_string + " > /sys/class/light_ccb/i2c_interface/i2c_w\""
#    print output_string
#    execute(output_string)
#    output_string = "adb shell \"echo 7 0x0091 0x04 0x02 " + "0x70" + " " + overall_string + " > /sys/class/light_ccb/i2c_interface/i2c_w\""
#    print output_string
#    execute(output_string)
    for i in xrange(0, len(num_pulses_list)):
        ...
        for j in xrange(0, 4): # four byte pulse counts
            ...
            output_string = "adb shell \"echo 3 0x0090 0x02 " + offset + " " + one_byte_hex(str(pulse_list[j])) + " > /sys/class/light_ccb/i2c_interface/i2c_w\""
            print output_string
            execute(output_string)
    
if args.pwm_test_cpld_start:
    # send the start signal
    start_string = "adb shell \"echo 3 0x0090 0x02 0x07 0x01 > /sys/class/light_ccb/i2c_interface/i2c_w\"" 
    print start_string
    execute(start_string)

if args.pwm_test_cpld_end:
    stop_string  = "adb shell \"echo 3 0x0090 0x02 0x08 0x01 > /sys/class/light_ccb/i2c_interface/i2c_w\"" 
    print stop_string
    execute(stop_string)
...
if args.cpld_stm_config:
    ...
    output_string = "adb shell \"echo 50 0x0092 "
    byte_count = 0
    ...
    for i in xrange(0, len(dutycycle_lists)):
        ...
        for j in xrange(0, 2): # two byte duty cycles
            ...
            byte_count += 1
            #output_string = "adb shell \"echo 3 0x0090 0x02 " + offset + " " + dutycycle_value + " > /sys/class/light_ccb/i2c_interface/i2c_w\""
            #print output_string
            #execute(output_string)
    ...
    for i in xrange(0, len(num_pulses_list)):
        ...
        for j in xrange(0, 4): # four byte pulse counts
            ...
            #output_string = "adb shell \"echo 3 0x0090 0x02 " + offset + " " + pulse_value + " > /sys/class/light_ccb/i2c_interface/i2c_w\""
            #print output_string
            #execute(output_string)
...
if args.cpld_stm_start:
    ...
    output_string = "adb shell \"echo 5 0x0093 " + m_bitmask + " 0x00 0x01 \
    > /sys/class/light_ccb/i2c_interface/i2c_w\""
    print output_string
    execute(output_string)
...
if args.read_hall_sensor_value:
    ...
    if args.lens:
        ...
        output_string = "adb shell \"echo 3 0x0040 " + m_bitmask + " > /sys/class/light_ccb/i2c_interface/i2c_w\""
        print output_string
        execute(output_string)

    if args.mirror:
        print "Reading mirror hall sensor..."
        output_string = "adb shell \"echo 3 0x0044 " + m_bitmask + " > /sys/class/light_ccb/i2c_interface/i2c_w\""
        print output_string
        execute(output_string)
...
if args.go_to_hall_sensor_value:
    ...
    if args.lens:
        ...
        args_len =  3 + (len(args.camera) * 2)
        output_string = "adb shell \"echo " + str(args_len)  +" 0x0040 " + m_bitmask + " " + \
    ...
    if args.mirror:
        ...
        output_string = "adb shell \"echo 5 0x0044 " + m_bitmask + " " + \
        destination_string + " > /sys/class/light_ccb/i2c_interface/i2c_w\""
        print output_string
        execute(output_string)
...
if args.fine_nudge:
    ...
    if args.camera:
        ...
        if args.lens:
            output_string = "adb shell \"echo 6 0x0051 " + m_bitmask + " " + \
            one_byte_hex(direction) + " " + two_byte_little_endian(hex(int(multiplier))) + \
            " " + "> /sys/class/light_ccb/i2c_interface/i2c_w\""
        if args.mirror:
            output_string = "adb shell \"echo 6 0x0052 " + m_bitmask + " " + \
            one_byte_hex(direction) + " " + two_byte_little_endian(hex(int(multiplier))) + \
            " " + "> /sys/class/light_ccb/i2c_interface/i2c_w\""
