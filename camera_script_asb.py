...
i2cpath = "/sys/class/i2c-adapter/i2c-11/11-0010/i2c_w"
ft4222 = "ft4222"
...
parser.add_argument("-auto", "--auto", help="turn on/off auto mode", action="store", choices=['on', 'off'])
parser.add_argument("-manual", "--manual", help="turn on/off manual mode", action="store", choices=['on', 'off'])
...
parser.add_argument("-tol", "--tolerance", help="tolerance value",
					action="store")
parser.add_argument("-burst", "--burst", help="burst value", action="store")
parser.add_argument("-a", "--asic", help="Turn on/off asic",action="store", choices=['0', '1'])
parser.add_argument("-ee", "--eeprom", nargs='+', help="read eeprom",action="store")
parser.add_argument("-z", "--zoom_factor", help="select zoom value",type=float,
					action="store")
...
if args.zoom_factor:
	if args.read:
		if args.ucid:
			...
			gain_string = "adb shell \"echo 4" + " 0x0000 0x5C 0x00 " + " " + ucid +\
				" > /sys/class/i2c-adapter/i2c-11/11-0010/i2c_w\""
			#print gain_string
			execute(gain_string)
			byte_count = 0
			read_string = "adb shell \"echo 4 0x005C "+"> /sys/class/i2c-adapter/i2c-11/11-0010/i2c_br\""
			#print read_string
			execute(read_string)
			#print "adb shell \"cat /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_br\""
			print (execute("adb shell \"cat /sys/class/i2c-adapter/i2c-11/11-0010/i2c_br\""))
			print ""
		else:
			print "Missing UCID"
			exit()
		...
		fstring = ""
		fstring += convert_to_hex_4byte(float_to_hex(args.zoom_factor))
		print "Total gain is " + str(args.gain)
		if args.ucid:
			ucid = ucid_to_hex(args.ucid)
			gain_string = "adb shell \"echo 8 0x0000 0x5C 0x00 " + ucid + " " + fstring + " > /sys/class/i2c-adapter/i2c-11/11-0010/i2c_w\""
		else:
			print "Missing UCID"
			exit()
		execute(gain_string)
...
if args.camera:
	...
	if args.open:
		...
		+ fstring + " > /sys/class/i2c-adapter/i2c-11/11-0010/i2c_w\""
		#print open_string
		execute(open_string)
		print ""
		if args.ucid:
			ucid_string = "adb shell 'echo 4 0x0000 0x00 0x10 " + ucid_to_hex(args.ucid) + " > /sys/class/i2c-adapter/i2c-11/11-0010/i2c_w'"
	...
	if args.auto is not None or args.manual is not None :
		...
		burst_string = "adb shell \"echo 3 0x0000 0x10 0x00 " + one_byte_hex(args.burst) + " > /sys/class/i2c-adapter/i2c-11/11-0010/i2c_w\""
		execute(burst_string)
		exit()
	elif args.asic:
		asic_string = "adb shell \"gpio_set -n 130 -s args.asic\""
		execute(asic_string)
		exit()
	else:
		if not args.ucid:
	...
	if args.camera:
		if args.read:
			...
			if args.ucid:
				...
				ucid + " " + fstring + " > /sys/class/i2c-adapter/i2c-11/11-0010/i2c_w\""
			else:
...
if args.eeprom:
	if args.camera:
		...
		for camera in args.camera:
			...
			fstring += " "
			byte_count += 4
		eeprom_string = "adb shell \"echo " + str(byte_count) + " 0xBEEF  0x72 0x02 " + m_bitmask + " "\
		+ fstring + " > /sys/class/i2c-adapter/i2c-11/11-0010/i2c_w\""
		#print gain_string
		print(eeprom_string)
		print ""
...
if args.go_to_hall_sensor_value:
	...
	if args.lens:
		...
		output_string = "adb shell \"echo " + str(args_len)  +" 0x0000 0x40 0x80 " + m_bitmask + " " + \
		destination_string + " > /sys/class/i2c-adapter/i2c-11/11-0010/i2c_w\""
	...
	if args.mirror:
		...
		output_string = "adb shell \"echo 7 0x0000 0x44 0x80 " + m_bitmask + " " + \
		destination_string + " > /sys/class/i2c-adapter/i2c-11/11-0010/i2c_w\""
		#print output_string
		execute(output_string)
